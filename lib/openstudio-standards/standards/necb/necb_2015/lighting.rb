class NECB2015
  ...
  def apply_lighting_schedule(space_type, space_type_properties,default_sch_set)
    ...
    c =1
      ...
      rules = model_find_objects(standards_data['schedules'], 'name' => occupancy_schedule) # returns all schedules with schedule name entered

      #check if it exists
      if rules.size.zero? #does not exist
        OpenStudio.logFree(OpenStudio::Error, 'openstudio.standards.Model', "Cannot find data for schedule: #{occupancy_schedule}.")
      else #exists

        #Create new lighting schedule
        lighting_sch_ruleset = OpenStudio::Model::ScheduleRuleset.new(space_type.model)
        lighting_sch_ruleset.setName("#{space_type.name.to_s} Lighting Schedule Ruleset")
        ...
        rules.each do|rule|
          ...
          if day_types.include?('Default')
            ...
            day_sch.setName("#{space_type.name.to_s} #{occupancy_schedule} Lighting Default")
            model_add_vals_to_sch(space_type.model, day_sch, sch_type, hourly_occ_control)
            a= 22
          elsif day_types.include?('Wknd') ||
            ...
            day_sch.setName("#{space_type.name.to_s} #{occupancy_schedule} #{day_types} Lighting Day")
            model_add_vals_to_sch(space_type.model, day_sch, sch_type, hourly_occ_control)
          ...
          elsif day_types.include?('WntrDsn')
            ...
            day_sch.setName("#{space_type.name.to_s} #{occupancy_schedule} Lighting Winter Design Day")
            model_add_vals_to_sch(space_type.model, day_sch, sch_type, hourly_occ_control)

          elsif day_types.include?('SmrDsn')
            day_sch = OpenStudio::Model::ScheduleDay.new(space_type.model)
            lighting_sch_ruleset.setSummerDesignDaySchedule(day_sch)
            day_sch = lighting_sch_ruleset.summerDesignDaySchedule
            day_sch.setName("#{space_type.name.to_s} #{occupancy_schedule} Lighting Summer Design Day")
            model_add_vals_to_sch(space_type.model, day_sch, sch_type, hourly_occ_control)
      ...
      unless orig_lighting_sch.nil?
        ...
        OpenStudio.logFree(OpenStudio::Info, 'openstudio.standards.SpaceType', "#{space_type.name.to_s} set lighting schedule to #{orig_lighting_sch}.")
      end
